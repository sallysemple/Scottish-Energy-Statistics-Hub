    ...
    tabsetPanel(
      tabPanel("Charts",
               fluidRow(column(8,
                               ...
                               selectInput(ns("YearSelect"), "Year:", c(2021:2004), selected = 2021, multiple = FALSE,
...
ElecGen <- function(input, output, session) {
  ...
  observeEvent(input$ToggleTable2, {
    toggle("ElecGenFuelScotPropTable")
  })
  ...
  output$ElecGenFuelPlot <- renderPlotly  ({
    ...
    if(as.numeric(input$YearSelect) >= 2017){
    ...
    p <-
      ...
      add_trace(
        ...
        text = paste0("Bioenergy and Waste: ", percent(ElecGenFuel$`Bioenergy and Waste`, accuracy = 0.1)),
        hoverinfo = 'text',
  ...
  output$ElecGenFuelTable = renderDataTable({
    ...
    datatable(
      ...
      formatStyle(c(7, 9, 15, 13), fontStyle = "italic") %>% 
